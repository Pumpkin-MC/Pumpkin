...
pub trait EventHandler<E> {
    fn handle<'a>(&'a self, event: E) -> E;
}

pub(crate) trait ErasedEventHandler: Send + Sync {
    fn handle_erased<'a>(&'a self, event: Event) -> Event;
}
...
{
    fn handle_erased(&self, event: Event) -> Event {
        let specific_event = E::from_event(event);
        self.handler.handle(specific_event).into_event()
    }
}
...
impl Context {
    ...
    >(
        ...
        self.register_event(id, event_priority, blocking);
        Ok(id)
    }
}
