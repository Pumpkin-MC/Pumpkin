...
impl StatusEffect {
    ...
    pub fn from_id(id: u8) -> Option<&'static Self> {
        match id {
            21 => Some(&Self::ABSORPTION),
            30 => Some(&Self::BAD_OMEN),
            14 => Some(&Self::BLINDNESS),
            28 => Some(&Self::CONDUIT_POWER),
            32 => Some(&Self::DARKNESS),
            29 => Some(&Self::DOLPHINS_GRACE),
            11 => Some(&Self::FIRE_RESISTANCE),
            23 => Some(&Self::GLOWING),
            2 => Some(&Self::HASTE),
            20 => Some(&Self::HEALTH_BOOST),
            31 => Some(&Self::HERO_OF_THE_VILLAGE),
            16 => Some(&Self::HUNGER),
            38 => Some(&Self::INFESTED),
            6 => Some(&Self::INSTANT_DAMAGE),
            5 => Some(&Self::INSTANT_HEALTH),
            13 => Some(&Self::INVISIBILITY),
            7 => Some(&Self::JUMP_BOOST),
            24 => Some(&Self::LEVITATION),
            25 => Some(&Self::LUCK),
            3 => Some(&Self::MINING_FATIGUE),
            8 => Some(&Self::NAUSEA),
            15 => Some(&Self::NIGHT_VISION),
            37 => Some(&Self::OOZING),
            18 => Some(&Self::POISON),
            34 => Some(&Self::RAID_OMEN),
            9 => Some(&Self::REGENERATION),
            10 => Some(&Self::RESISTANCE),
            22 => Some(&Self::SATURATION),
            27 => Some(&Self::SLOW_FALLING),
            1 => Some(&Self::SLOWNESS),
            0 => Some(&Self::SPEED),
            4 => Some(&Self::STRENGTH),
            33 => Some(&Self::TRIAL_OMEN),
            26 => Some(&Self::UNLUCK),
            12 => Some(&Self::WATER_BREATHING),
            17 => Some(&Self::WEAKNESS),
            36 => Some(&Self::WEAVING),
            35 => Some(&Self::WIND_CHARGED),
            19 => Some(&Self::WITHER),
            _ => None,
        }
    }
