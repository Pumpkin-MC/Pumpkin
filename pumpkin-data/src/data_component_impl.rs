...
impl Hash for AttackRangeImpl {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.min_range.to_bits().hash(state);
        self.max_range.to_bits().hash(state);
        self.min_creative_range.to_bits().hash(state);
        self.max_creative_range.to_bits().hash(state);
