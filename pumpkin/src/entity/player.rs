...
pub struct Player {
    ...
    /// The item stack currently being held by the player.
...
impl Player {
    ...
    pub async fn attack(&self, victim: Arc<dyn EntityBase>) {
        ...
        // The weapon cost is computed (cost = 1 or 2) with item stack locked, then damage_held_item
        // re-acquires the lock. In async multi-task scenarios, another task could theoretically
        // swap the held item stack between these operations, causing the cost to apply to the wrong item_stack.
    ...
    /// Returns the durability cost for using the held item stack as a weapon in combat.
    ...
    /// Applies `amount` durability damage to the item stack in `slot`.
    /// Broadcasts an [`EntityStatus`] break event and syncs the slot if the item stack is destroyed.
    pub async fn damage_item_in_slot(&self, slot: &EquipmentSlot, amount: i32) -> bool {
        ...
        if let Some((result, updated_stack)) = updated {
            ...
            // use the item stack texture for break particles.
    ...
    /// Convenience wrapper – damages the currently held (main-hand) item stack.
...
impl NBTStorage for EnderChestInventory {
    fn write_nbt<'a>(&'a self, nbt: &'a mut NbtCompound) -> NbtFuture<'a, ()> {
        Box::pin(async {
            // Create item stack list with the correct capacity (inventory size)
