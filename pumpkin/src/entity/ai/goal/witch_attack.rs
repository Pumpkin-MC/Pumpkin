...
impl WitchAttackGoal {
    ...
    async fn throw_harming(mob: &dyn Mob, target: &dyn EntityBase) {
        ...
        let entity = Entity::new(world.clone(), eye, &EntityType::SPLASH_POTION);
        let splash = SplashPotionEntity::new_shot(entity, shooter);

        // Harming splash (vanilla primary offensive potion).
        ...
        splash
            .thrown
            .set_velocity_from(shooter, pitch, yaw, -20.0, 0.75, 8.0);
    ...
    fn tick<'a>(&'a mut self, mob: &'a dyn Mob) -> GoalFuture<'a, ()> {
        Box::pin(async {
            ...
            if self.attack_time <= 0 && dist_sq <= ATTACK_RADIUS_SQ {
                Self::throw_harming(mob, target.as_ref()).await;
