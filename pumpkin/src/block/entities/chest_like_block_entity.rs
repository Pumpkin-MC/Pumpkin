    ...
    ($struct_name:ty) => {
        impl $crate::block::entities::BlockEntity for $struct_name {
                ...
                Self: Sized,
            {
                ...
                if chest.loot_table.lock().expect(&$crate::localized_log("debug.expect.loot_table_mutex_not_poisoned")).is_none() {
            ...
            fn write_nbt<'a>(
                ...
                Box::pin(async move {
                    ...
                    let loot_table_key = {
                        let guard = self.loot_table.lock().expect(&$crate::localized_log("debug.expect.loot_table_mutex_not_poisoned"));
                        guard.clone()
                    };
            ...
            fn take_loot_table(&self) -> Option<(String, i64)> {
                let mut guard = self.loot_table.lock().expect(&$crate::localized_log("debug.expect.loot_table_mutex_not_poisoned"));
                guard.take().map(|key| (key, self.loot_table_seed))
            }

            fn has_loot_table(&self) -> bool {
                self.loot_table.lock().expect(&$crate::localized_log("debug.expect.loot_table_mutex_not_poisoned")).is_some()
            }
        }
    };
}
