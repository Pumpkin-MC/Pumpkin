            ...
            async fn can_accept_recipe_output(
                ...
                if let Some(recipe_output_item) = pumpkin_data::item::Item::from_registry_key(
                    recipe
                        ...
                        .expect(&$crate::localized_log(
                            "debug.expect.recipe_id_missing_prefix",
                        )),
                ) && !is_top_items_empty
                ...
                if let Some(recipe) = recipe {
                    if can_accept_output {
                        ...
                        let Some(output_item) = pumpkin_data::item::Item::from_registry_key(
                            recipe.result.id.strip_prefix("minecraft:").expect(
                                &$crate::localized_log("debug.expect.recipe_id_missing_prefix"),
                            ),
                        ) else {
                            return false;
                        };
