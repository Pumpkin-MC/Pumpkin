...
pub async fn drop_loot(
    ...
    let datapack_handled = if let Some(server) = world.server.upgrade() {
        ...
        if let Some(dp_table) = dp_tables.get(&dp_id) {
            ...
            let dp_results = pumpkin_datapack::loot::evaluate::evaluate_loot_table(dp_table, &ctx);
    ...
    if !datapack_handled && let Some(loot_table) = &block.loot_table {
        for stack in loot_table.get_loot(params.clone()) {
            world.drop_stack(pos, stack).await;
        }
    }
