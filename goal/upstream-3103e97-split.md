...
| 1 | Java net write-path: replace `.unwrap()` with log+return/continue; keep_alive `unwrap_or_default` | Low | In progress |
...
| 5 | protocol `ServerPacket<'a>` + ser `NetworkReadExt` + all packet reads | High (80+ files) | Later as one PR |
