...
impl<T: Send + Sync + 'static> RegistryBuilder<T> {
    ...
    pub fn new_static(
        ...
        let (added_entries, added_mapping) = BOOTSTRAP.get().ok_or(BootstrapError::Uninitialized).and_then(|m| m.populate::<T>(name))?;
    ...
    pub fn frozen(name: &Identifier) -> Result<FrozenRegistry<T>, BootstrapError> {
        let (entries, mapping) = BOOTSTRAP.get().ok_or(BootstrapError::Uninitialized).and_then(|m| m.populate::<T>(name))?;
