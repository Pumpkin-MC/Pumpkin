...
use pumpkin_registry::{BOOTSTRAP, bootstrap::{BootstrapManager, RegistryEntry}, bootstrap_provider};
...
fn discovers_linker_section_providers() {
    ...
    assert_eq!(BOOTSTRAP.get().unwrap().providers_for(&BLOCK_REGISTRY).count(), 3,);
}
...
fn populates_all_linker_section_providers() {
    ...
    let (entries, mapping) = BOOTSTRAP.get().unwrap().populate::<Block>(&BLOCK_REGISTRY).unwrap();
