...
/// Virtual pack that exposes Pumpkin's embedded vanilla datapack as pack resources.
///
...
impl PackResources for VanillaPackResources {
    ...
    fn get_resource(&self, namespace: &str, path: &str) -> Option<Vec<u8>> {
        crate::embedded_vanilla_datapack::get_vanilla_resource(namespace, path).map(<[u8]>::to_vec)
    }

    fn list_resources(&self, namespace: &str, prefix: &str) -> Vec<String> {
        let paths = crate::embedded_vanilla_datapack::list_vanilla_resources(namespace, prefix);
        paths.iter().map(|s| (*s).to_string()).collect()
    }

    fn get_namespaces(&self) -> Vec<String> {
        crate::embedded_vanilla_datapack::get_vanilla_namespaces()
            .iter()
            .map(|s| (*s).to_string())
            .collect()
    }
...
#[cfg(test)]
mod tests {
    use super::{PackResources, VanillaPackResources};

    #[test]
    fn vanilla_pack_contains_registry_resources() {
        let pack = VanillaPackResources;

        for path in [
            "damage_type/generic.json",
            ...
            assert!(
                pack.get_resource("minecraft", path).is_some(),
                "missing embedded vanilla resource minecraft:{path}",
            );
        }
    }
}
