...
impl MinecartEntity {
    ...
    pub async fn container_comparator_output(&self) -> Option<u8> {
...
impl EntityBase for MinecartEntity {
    ...
    fn tick<'a>(
        ...
        Box::pin(async move {
            ...
            let has_passengers = !self.vehicle.entity.passengers.lock().await.is_empty();
            ...
            if step.length() > 0.001 || (is_on_rails && power_track) {
                ...
                self.move_entity(caller, step).await;
                ...
                if is_on_rails
                    && ((step.x - velocity.x).abs() > 1.0e-9
                        || (step.z - velocity.z).abs() > 1.0e-9)
                {
                    ...
                    let blocked =
                        |requested: f64, achieved: f64| (requested - achieved).abs() > 1.0e-9;
