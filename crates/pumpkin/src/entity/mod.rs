...
impl Entity {
    ...
    // so for versions before 1.21 we can only safely forward the shared-flags byte
    // index 0, type byte, identical in every supported Java version, which carries
    // the on-fire, sneaking, sprinting, and glowing states.
    fn can_send_tracked_data(id: tracked_data::TrackedId, version: JavaMinecraftVersion) -> bool {
        version >= JavaMinecraftVersion::V_1_21
            || id == tracked_data::entity::DATA_SHARED_FLAGS_ID.id
    }
