...
impl EntityBase for TNTEntity {
    ...
    /// entity is serialized, and [`from_type`](crate::entity::type::from_type) rebuilds it on
    /// load with the default 80 ticks: a chunk reload or relog resets a nearly-spent fuse, so
    /// the TNT explodes late.
    ...
    fn tick(&self, caller: &Arc<dyn EntityBase>, server: &Server) {
        ...
        // an either/or choice. The velocity is read back rather than reusing the pre-move
        // value: `move_entity` clamps on collision, and an explosion may have pushed us
        // while we were moving above.
