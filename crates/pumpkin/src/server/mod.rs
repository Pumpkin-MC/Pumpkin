    ...
    pub async fn push_bedrock_skin_pack(
        ...
        for world in self.worlds.load().iter() {
            for player in world.players.load().iter() {
                // World publication precedes Java's CLogin. Initial spawning
                // reconciles the latest revision after it is safe to offer it.
                if player.bedrock_player_tracking_ready.load(Ordering::Acquire)
