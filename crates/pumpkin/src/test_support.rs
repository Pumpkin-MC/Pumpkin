...
use crate::net::java::{JavaClient, TestJavaPacketQueue, pending::PendingConnection};
...
impl TestServer {
    ...
    pub(crate) async fn new_java_player(&self) -> Arc<Player> {
        self.create_java_player(false).await.0
    }

    pub(crate) async fn new_java_player_with_packets(&self) -> (Arc<Player>, TestJavaPacketQueue) {
        let (player, packets) = self.create_java_player(true).await;
        (player, packets.unwrap())
    }

    async fn create_java_player(
        &self,
        capture_packets: bool,
    ) -> (Arc<Player>, Option<TestJavaPacketQueue>) {
        ...
        let packets = capture_packets.then(|| client.take_outgoing_packets_for_test());
        client
            .version
