...
/// Registers the pack's per-locale advancement strings with the server's translations.
///
/// Bedrock resolves the chat announcement client-side from this pack, but `CToastRequest` carries
/// plain strings, so a toast has to be rendered with text the server looked up itself.
        ...
        for (locale, lang) in lang_files() {
            for line in lang.lines() {
                ...
                if key.starts_with("advancements.") {
                    // Advancement components resolve through the `minecraft:` namespace.
...
/// Locates the pack's central directory, which lists every entry.
fn central_directory() -> Option<(usize, usize)> {
    // The end of central directory record is followed by an optional comment of up to 64 KiB.
...
/// A central directory entry, with the offset of the next one.
struct Entry<'a> {
    name: &'a [u8],
    method: usize,
...
impl Entry<'_> {
    /// The locale of a `texts/<locale>.lang` entry, if this entry is one.
    fn locale(&self) -> Option<Locale> {
        ...
        if locale == Locale::EnUs && !code.eq_ignore_ascii_case("en_us") {
            // `Locale::from_str` answers English for codes it does not know, which would file
            // another language's strings under English.
    ...
    /// The decompressed contents of this entry, read from its local file header.
...
mod tests {
    ...
    /// Every language Bedrock ships has to reach the server, so this walks the pack's own files:
    /// each one must map to its locale and resolve back to the strings it carries, both the
    /// advancement title the splash names and the frame label it is headed with.
    #[test]
