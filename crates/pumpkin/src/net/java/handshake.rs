use pumpkin_data::{
    packet::{HIGHEST_SUPPORTED_MC_VERSION, LOWEST_SUPPORTED_MC_VERSION},
    translation,
};
        ...
        if self.connection_state.load() != ConnectionState::Status {
            ...
            if protocol < LOWEST_SUPPORTED_MC_VERSION.protocol_version() as u32 {
                self.kick(TextComponent::translate_cross(
                    ...
                    [TextComponent::text(
                        HIGHEST_SUPPORTED_MC_VERSION.to_string(),
                    )],
                ))
                .await;
            } else if protocol > HIGHEST_SUPPORTED_MC_VERSION.protocol_version() as u32 {
                self.kick(TextComponent::translate_cross(
