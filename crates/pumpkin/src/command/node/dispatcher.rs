...
impl CommandDispatcher {
    ...
    pub fn disable_command(&mut self, name: impl Into<String>) {
        self.disabled.insert(name.into().to_ascii_lowercase());
    }

    /// Makes a plugin command unavailable until it is registered again.
    fn deactivate_plugin_command(&mut self, name: impl Into<String>) {
    ...
    pub fn tree_alias_names(&self, primary: &str) -> Vec<String> {
        let Some(primary_id) = self.command_node_id(primary) else {
            return Vec::new();
        };
    ...
    pub fn primary_command_name(&self, name: &str) -> String {
        if let Some(node_id) = self.command_node_id(name) {
    ...
    fn command_node_id(&self, name: &str) -> Option<CommandNodeId> {
        let name = name.to_ascii_lowercase();
        self.tree
            .get_root_children()
            .into_iter()
            .find(|node_id| self.tree[*node_id].meta.literal_lowercase == name)
            .or_else(|| self.tree.get(&name))
    }
...
mod test {
    ...
    #[test]
    fn plugin_commands_are_reactivated_when_registered_again() {
        let mut dispatcher = CommandDispatcher::new();
        let executor: fn(&CommandContext) -> CommandExecutorResult = |_| Ok(1);
        let command = || {
        ...
        dispatcher.deactivate_plugin_command_and_aliases("plugin-command");
        assert!(
            dispatcher
                .execute_input("plugin-alias", &source)
                .is_err_and(|error| error.error_type == &DISPATCHER_UNKNOWN_COMMAND)
