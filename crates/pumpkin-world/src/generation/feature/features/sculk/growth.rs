...
impl GrowthRules {
            ...
            for dz in -GROWTH_CHECK_RADIUS..=GROWTH_CHECK_RADIUS {
                for dy in 0..=GROWTH_CHECK_HEIGHT {
                    ...
                    if let Some(s) = level.sculk_get(check_pos) {
                        ...
                        if id == BlockId::SCULK_SENSOR
                            || id == BlockId::CALIBRATED_SCULK_SENSOR
                            || id == BlockId::SCULK_SHRIEKER
                        {
                            growth_count += 1;
    ...
    pub fn random_growth_state(
        ...
        let base_state = if random.next_bounded_i32(11) == 0 {
            ...
            let mut props = SculkShriekerLikeProperties::default(
        ...
        // Apply waterlogging if the position holds water (shrieker only).
        if level.sculk_is_water_source(pos)
            && base_state.id.to_block_id() == BlockId::SCULK_SHRIEKER
        {
