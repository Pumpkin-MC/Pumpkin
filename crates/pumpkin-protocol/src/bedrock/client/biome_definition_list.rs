...
static BIOME_DEFINITIONS: LazyLock<Result<Vec<u8>, String>> = LazyLock::new(|| {
    encode_biome_definitions(include_bytes!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../assets/bedrock/biome_definitions.nbt"
    )))
...
impl PacketWrite for CBiomeDefinitionList {
    fn write<W: Write>(&self, writer: &mut W) -> Result<(), Error> {
        writer.write_all(
            BIOME_DEFINITIONS
                .as_ref()
                .map_err(|error| Error::new(ErrorKind::InvalidData, error.clone()))?,
        )
    }
}
...
fn encode_biome_definitions(document: &[u8]) -> Result<Vec<u8>, Error> {
...
mod tests {
    ...
    #[test]
    fn bundled_registry_encodes_once() {
        let payload = BIOME_DEFINITIONS
            .as_ref()
            .unwrap_or_else(|error| panic!("bundled biome definitions must encode: {error}"));
        let biome_count = VarUInt::read(&mut payload.as_slice())
            .unwrap_or_else(|error| panic!("biome count must decode: {error}"));
