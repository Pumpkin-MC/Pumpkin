...
mod tests {
    ...
    #[test]
    fn read_slice_borrows_string_and_advances_payload() {
        let bytes = [3, b'c', b'o', b'd', 1];
        let mut payload = bytes.as_slice();

        let string = <&str>::read_slice(&mut payload).unwrap();

        assert_eq!(string, "cod");
        assert_eq!(string.as_ptr(), bytes[1..].as_ptr());
        assert!(bool::read_slice(&mut payload).unwrap());
