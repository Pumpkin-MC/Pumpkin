...
use crate::lighting::storage::{get_block_light, set_block_light, get_sky_light, set_sky_light};
...
impl LightProvider for BlockLightProvider {
    #[inline(always)]
    fn get_light(cache: &Cache, pos: BlockPos) -> u8 { get_block_light(cache, pos) }
    #[inline(always)]
    fn set_light(cache: &mut Cache, pos: BlockPos, level: u8) { set_block_light(cache, pos, level) }
    #[inline(always)]
...
impl LightProvider for SkyLightProvider {
    #[inline(always)]
    fn get_light(cache: &Cache, pos: BlockPos) -> u8 { get_sky_light(cache, pos) }
    #[inline(always)]
    fn set_light(cache: &mut Cache, pos: BlockPos, level: u8) { set_sky_light(cache, pos, level) }
    #[inline(always)]
    fn propagate_level(current_level: u8, opacity: u8, dir: BlockDirection) -> u8 {
...
impl<P: LightProvider> LightPropagator<P> {
    ...
    pub fn propagate(&mut self, cache: &mut Cache) {
        ...
        while let Some(pos) = self.queue.pop_front() {
            let current_light = self.shadow_cache.get(&pos).copied()
            ...
            for dir in BlockDirection::all() {
                ...
                if rel_x < 0 || rel_x >= cache.size || rel_z < 0 || rel_z >= cache.size { continue; }
                ...
                let neighbor_light = self.shadow_cache.get(&neighbor_pos).copied()
                ...
                if new_level > neighbor_light {
                    ...
                    self.pending_updates.entry((chunk_x, chunk_z)).or_default().push((neighbor_pos, new_level));
        ...
        while let Some((pos, old_val)) = self.decrease_queue.pop_front() {
            for dir in BlockDirection::all() {
                 ...
                 if neighbor_light == predicted || neighbor_light < old_val {
                     ...
                     self.decrease_queue.push_back((neighbor_pos, neighbor_light));
