...
impl BrewingStandBlockEntity {
    ...
    async fn is_brewable(&self, ingredient: &ItemStack) -> bool {
        ...
        for slot_idx in 0..3usize {
            ...
            // Check item stack recipes first (potion -> splash potion, splash -> lingering)
    ...
    async fn do_brew(&self, world: &Arc<dyn crate::world::SimpleWorld>, ingredient: &ItemStack) {
        ...
        for slot_idx in 0..3usize {
            ...
            // Try item stack recipes first (potion -> splash/lingering)
            for recipe in ITEM_RECIPES.iter() {
                if slot.get_item().id == recipe.from().id
                    && recipe.ingredient().iter().any(|i| i.id == ingredient_id)
                {
            ...
            // Try potion recipes (modify potion type) if item stack recipe didn't apply
        ...
        // TODO: Implement remainder handling when item stack lookup by ID is available
    }
}
...
impl crate::inventory::Inventory for BrewingStandBlockEntity {
    ...
    fn is_valid_slot_for(&self, slot: usize, stack: &ItemStack) -> bool {
        ...
        match slot {
            ...
            3 => {
                // Check if item stack is a valid brewing ingredient
                ...
                // Allow any item stack that's not fuel (ingredient validation happens during brewing)
                true
            }
            // Slot 4 - fuel
            4 => stack
...
impl crate::block::entities::BlockEntity for BrewingStandBlockEntity {
        ...
        Self: Sized,
    {
        ...
        // If there's an ingredient in slot 3, remember its base item stack for matching
    ...
    fn tick<'a>(
        ...
        Box::pin(async move {
            // Refill fuel counter from fuel item stack if needed
            ...
            for (i, slot_arc) in self.items.iter().take(3).enumerate() {
                ...
                // Consider a potion slot "present" when it has an item stack and a PotionContents component or is a glass bottle
