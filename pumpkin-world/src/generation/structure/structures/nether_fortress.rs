...
use pumpkin_data::{Block, BlockState, block_properties::BlockProperties};
use pumpkin_util::{
    BlockDirection,
...
impl FortressPiece {
    ...
    fn to_boxed(&self) -> Box<dyn StructurePieceBase> {
        match self {
            FortressPiece::Bridge(p) => Box::new(p.clone()),
            FortressPiece::BridgeCrossing(p) => Box::new(p.clone()),
            FortressPiece::BridgeSmallCrossing(p) => Box::new(p.clone()),
            FortressPiece::BridgeStairs(p) => Box::new(p.clone()),
            FortressPiece::BridgePlatform(p) => Box::new(p.clone()),
            FortressPiece::BridgeEnd(p) => Box::new(p.clone()),
            FortressPiece::CorridorExit(p) => Box::new(p.clone()),
            FortressPiece::SmallCorridor(p) => Box::new(p.clone()),
            FortressPiece::CorridorCrossing(p) => Box::new(p.clone()),
            FortressPiece::CorridorRightTurn(p) => Box::new(p.clone()),
            FortressPiece::CorridorLeftTurn(p) => Box::new(p.clone()),
            FortressPiece::CorridorStairs(p) => Box::new(p.clone()),
            FortressPiece::CorridorBalcony(p) => Box::new(p.clone()),
            FortressPiece::CorridorNetherWartsRoom(p) => Box::new(p.clone()),
        }
    }

    fn bounding_box(&self) -> BlockBox {
        match self {
            FortressPiece::Bridge(p) => p.piece.piece.bounding_box,
            FortressPiece::BridgeCrossing(p) => p.piece.piece.bounding_box,
            FortressPiece::BridgeSmallCrossing(p) => p.piece.piece.bounding_box,
            FortressPiece::BridgeStairs(p) => p.piece.piece.bounding_box,
            FortressPiece::BridgePlatform(p) => p.piece.piece.bounding_box,
            FortressPiece::BridgeEnd(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorExit(p) => p.piece.piece.bounding_box,
            FortressPiece::SmallCorridor(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorCrossing(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorRightTurn(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorLeftTurn(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorStairs(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorBalcony(p) => p.piece.piece.bounding_box,
            FortressPiece::CorridorNetherWartsRoom(p) => p.piece.piece.bounding_box,
        }
    }
}
...
impl StructurePieceBase for CorridorStairsPiece {
    ...
    fn place(&mut self, chunk: &mut ProtoChunk, _random: &mut RandomGenerator, _seed: i64) {
        ...
        let bricks = Block::NETHER_BRICKS.default_state;
        let air = Block::AIR.default_state;
        let stairs = Block::NETHER_BRICK_STAIRS.default_state; // TODO: add facing
