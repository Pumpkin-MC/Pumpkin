...
- `advancement_translations.mcpack`
    - A Bedrock resource pack containing the Java `advancements.*` keys absent from Bedrock's own language files. It lets Bedrock resolve advancement announcements entirely client-side, and it is also the source Pumpkin reads to render advancement toasts, which the Bedrock protocol carries as plain strings rather than translation components.
