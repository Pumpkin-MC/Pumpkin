...
impl ToTokens for Locale {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Self::AfZa => quote!(Locale::AfZa),
            Self::ArSa => quote!(Locale::ArSa),
            Self::AstEs => quote!(Locale::AstEs),
            Self::AzAz => quote!(Locale::AzAz),
            Self::BaRu => quote!(Locale::BaRu),
            Self::Bar => quote!(Locale::Bar),
            Self::BeBy => quote!(Locale::BeBy),
            Self::BgBg => quote!(Locale::BgBg),
            Self::BrFr => quote!(Locale::BrFr),
            Self::Brb => quote!(Locale::Brb),
            Self::BsBa => quote!(Locale::BsBa),
            Self::CaEs => quote!(Locale::CaEs),
            Self::CsCz => quote!(Locale::CsCz),
            Self::CyGb => quote!(Locale::CyGb),
            Self::DaDk => quote!(Locale::DaDk),
            Self::DeAt => quote!(Locale::DeAt),
            Self::DeCh => quote!(Locale::DeCh),
            Self::DeDe => quote!(Locale::DeDe),
            Self::ElGr => quote!(Locale::ElGr),
            Self::EnAu => quote!(Locale::EnAu),
            Self::EnCa => quote!(Locale::EnCa),
            Self::EnGb => quote!(Locale::EnGb),
            Self::EnNz => quote!(Locale::EnNz),
            Self::EnPt => quote!(Locale::EnPt),
            Self::EnUd => quote!(Locale::EnUd),
            Self::EnUs => quote!(Locale::EnUs),
            Self::Enp => quote!(Locale::Enp),
            Self::Enws => quote!(Locale::Enws),
            Self::EoUy => quote!(Locale::EoUy),
            Self::EsAr => quote!(Locale::EsAr),
            Self::EsCl => quote!(Locale::EsCl),
            Self::EsEc => quote!(Locale::EsEc),
            Self::EsEs => quote!(Locale::EsEs),
            Self::EsMx => quote!(Locale::EsMx),
            Self::EsUy => quote!(Locale::EsUy),
            Self::EsVe => quote!(Locale::EsVe),
            Self::Esan => quote!(Locale::Esan),
            Self::EtEe => quote!(Locale::EtEe),
            Self::EuEs => quote!(Locale::EuEs),
            Self::FaIr => quote!(Locale::FaIr),
            Self::FiFi => quote!(Locale::FiFi),
            Self::FilPh => quote!(Locale::FilPh),
            Self::FoFo => quote!(Locale::FoFo),
            Self::FrCa => quote!(Locale::FrCa),
            Self::FrFr => quote!(Locale::FrFr),
            Self::FraDe => quote!(Locale::FraDe),
            Self::FurIt => quote!(Locale::FurIt),
            Self::FyNl => quote!(Locale::FyNl),
            Self::GaIe => quote!(Locale::GaIe),
            Self::GdGb => quote!(Locale::GdGb),
            Self::GlEs => quote!(Locale::GlEs),
            Self::HawUs => quote!(Locale::HawUs),
            Self::HeIl => quote!(Locale::HeIl),
            Self::HiIn => quote!(Locale::HiIn),
            Self::HrHr => quote!(Locale::HrHr),
            Self::HuHu => quote!(Locale::HuHu),
            Self::HyAm => quote!(Locale::HyAm),
            Self::IdId => quote!(Locale::IdId),
            Self::IgNg => quote!(Locale::IgNg),
            Self::IoEn => quote!(Locale::IoEn),
            Self::IsIs => quote!(Locale::IsIs),
            Self::Isv => quote!(Locale::Isv),
            Self::ItIt => quote!(Locale::ItIt),
            Self::JaJp => quote!(Locale::JaJp),
            Self::JboEn => quote!(Locale::JboEn),
            Self::KaGe => quote!(Locale::KaGe),
            Self::KkKz => quote!(Locale::KkKz),
            Self::KnIn => quote!(Locale::KnIn),
            Self::KoKr => quote!(Locale::KoKr),
            Self::Ksh => quote!(Locale::Ksh),
            Self::KwGb => quote!(Locale::KwGb),
            Self::LaLa => quote!(Locale::LaLa),
            Self::LbLu => quote!(Locale::LbLu),
            Self::LiLi => quote!(Locale::LiLi),
            Self::Lmo => quote!(Locale::Lmo),
            Self::LoLa => quote!(Locale::LoLa),
            Self::LolUs => quote!(Locale::LolUs),
            Self::LtLt => quote!(Locale::LtLt),
            Self::LvLv => quote!(Locale::LvLv),
            Self::Lzh => quote!(Locale::Lzh),
            Self::MkMk => quote!(Locale::MkMk),
            Self::MnMn => quote!(Locale::MnMn),
            Self::MsMy => quote!(Locale::MsMy),
            Self::MtMt => quote!(Locale::MtMt),
            Self::Nah => quote!(Locale::Nah),
            Self::NdsDe => quote!(Locale::NdsDe),
            Self::NlBe => quote!(Locale::NlBe),
            Self::NlNl => quote!(Locale::NlNl),
            Self::NnNo => quote!(Locale::NnNo),
            Self::NoNo => quote!(Locale::NoNo),
            Self::OcFr => quote!(Locale::OcFr),
            Self::Ovd => quote!(Locale::Ovd),
            Self::PlPl => quote!(Locale::PlPl),
            Self::PtBr => quote!(Locale::PtBr),
            Self::PtPt => quote!(Locale::PtPt),
            Self::QyaAa => quote!(Locale::QyaAa),
            Self::RoRo => quote!(Locale::RoRo),
            Self::Rpr => quote!(Locale::Rpr),
            Self::RuRu => quote!(Locale::RuRu),
            Self::RyUa => quote!(Locale::RyUa),
            Self::SahSah => quote!(Locale::SahSah),
            Self::SeNo => quote!(Locale::SeNo),
            Self::SkSk => quote!(Locale::SkSk),
            Self::SlSi => quote!(Locale::SlSi),
            Self::SoSo => quote!(Locale::SoSo),
            Self::SqAl => quote!(Locale::SqAl),
            Self::SrCs => quote!(Locale::SrCs),
            Self::SrSp => quote!(Locale::SrSp),
            Self::SvSe => quote!(Locale::SvSe),
            Self::Sxu => quote!(Locale::Sxu),
            Self::Szl => quote!(Locale::Szl),
            Self::TaIn => quote!(Locale::TaIn),
            Self::ThTh => quote!(Locale::ThTh),
            Self::TlPh => quote!(Locale::TlPh),
            Self::TlhAa => quote!(Locale::TlhAa),
            Self::Tok => quote!(Locale::Tok),
            Self::TrTr => quote!(Locale::TrTr),
            Self::TtRu => quote!(Locale::TtRu),
            Self::UkUa => quote!(Locale::UkUa),
            Self::ValEs => quote!(Locale::ValEs),
            Self::VecIt => quote!(Locale::VecIt),
            Self::ViVn => quote!(Locale::ViVn),
            Self::YiDe => quote!(Locale::YiDe),
            Self::YoNg => quote!(Locale::YoNg),
            Self::ZhCn => quote!(Locale::ZhCn),
            Self::ZhHk => quote!(Locale::ZhHk),
            Self::ZhTw => quote!(Locale::ZhTw),
            Self::ZlmArab => quote!(Locale::ZlmArab),
        });
    }
}
